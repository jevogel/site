...
class RequestChat extends Component {
  ...
  componentWillMount() {
    ...
    db.onRequestMessageAdded(snapshot => {
  ...
  onAddMessage(event) {
    ...
    db.doCreateRequestMessage(authUser, value, timestamp, sort_key);
  ...
  render() {
    ...
    return (
      <div>
        <div className="chat" id="announcements">
          ...
          <form onSubmit={this.onAddMessage}>
            <input
              ...
              placeholder="Submit project request..."
            />
          </form>
        </div>
      </div>
